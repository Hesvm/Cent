...
export function TransactionDetail({ transaction, onClose }: TransactionDetailProps) {
  ...
  const [pendingScopedUpdate, setPendingScopedUpdate] = useState<Partial<Transaction> | null>(null)
  const [showRecurringEditScope, setShowRecurringEditScope] = useState(false)
  const [showAutoDeleteScope, setShowAutoDeleteScope] = useState(false)
  const backdropRef = useRef<HTMLDivElement>(null)
  const sheetRef = useRef<HTMLDivElement>(null)

  const isIncome = transaction.type === 'income'
  const isRecurringTemplate = Boolean(transaction.recurring && !transaction.isAutoGenerated)
  const autoGeneratedLabel = transaction.parentRecurringId
    ? capitalize(transaction.frequency === 'none' ? 'recurring' : transaction.frequency)
    : null
  ...
  async function handleDelete() {
    try {
      if (transaction.isAutoGenerated && transaction.parentRecurringId) {
        setShowAutoDeleteScope(true)
        return
      }
  ...
  async function applyUpdate(updates: Partial<Transaction>) {
    if (isRecurringTemplate) {
      setPendingScopedUpdate(updates)
  ...
  async function commitName() {
    ...
    if (trimmed && trimmed !== transaction.name) {
      await applyUpdate({ name: trimmed })
    } else {
  ...
  async function commitAmount() {
    ...
    if (!isNaN(parsed) && parsed > 0 && parsed !== transaction.amount) {
      await applyUpdate({ amount: parsed })
    } else {
  ...
  async function handleCategorySelect(cat: string | null) {
    ...
    if (cat !== transaction.category) {
      await applyUpdate({ category: cat })
    }
  }

  async function handleScopedEdit(scope: 'single' | 'future') {
    if (!pendingScopedUpdate) return
    try {
      await updateTransaction(transaction.id, pendingScopedUpdate, scope)
  ...
  async function handleAutoDelete(mode: 'single' | 'stop-recurring') {
    try {
      await deleteTransaction(transaction.id, mode)
      setShowAutoDeleteScope(false)
      close()
    } catch {
      setDeleteError(true)
      setTimeout(() => setDeleteError(false), 3000)
      ...
      >
        ...
        >
          ...
          <div className="px-6 pb-8">
            ...
            {showRecurringEditScope && pendingScopedUpdate && (
              <div className="rounded-xl bg-[#F8F8F8] px-4 py-3 mb-3">
                <p className="text-[13px] text-text-primary font-medium mb-2">Update just this entry or future ones too?</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleScopedEdit('single')}
                    className="flex-1 rounded-pill py-2 text-[12px] font-semibold font-rounded bg-bg-card text-text-primary"
                  >
                    Edit this only
                  </button>
                  <button
                    onClick={() => handleScopedEdit('future')}
                    className="flex-1 rounded-pill py-2 text-[12px] font-semibold font-rounded text-white bg-[#1A1A1A]"
                  >
                    Edit all future
                  </button>
                </div>
            ...
            {showAutoDeleteScope && (
              <div className="rounded-xl bg-[#F8F8F8] px-4 py-3 mb-3">
                <p className="text-[13px] text-text-primary font-medium mb-2">Remove this recurring entry how?</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleAutoDelete('single')}
                    className="flex-1 rounded-pill py-2 text-[12px] font-semibold font-rounded bg-bg-card text-text-primary"
                  >
                    Skip this month
                  </button>
                  <button
                    onClick={() => handleAutoDelete('stop-recurring')}
                    className="flex-1 rounded-pill py-2 text-[12px] font-semibold font-rounded text-white bg-[#1A1A1A]"
                  >
                    Stop recurring
                  </button>
            ...
            <div className="mb-1">
              {editingName ? (
                <input
                  ...
                  className="w-full text-center text-[22px] font-bold text-text-primary bg-transparent focus:outline-none border-b border-border font-rounded"
                />
              ) : (
                <button
                  onClick={() => setEditingName(true)}
                  className="block w-full text-center text-[22px] font-bold text-text-primary hover:opacity-70 transition-opacity font-rounded"
                >
                  {transaction.name}
                </button>
              )}
              {transaction.isAutoGenerated && autoGeneratedLabel && (
                <div className="flex justify-center mt-2">
                  <span className="text-[11px] text-text-secondary font-rounded px-2 py-1 rounded-pill bg-[#F8F8F8]">
                    ↻ {autoGeneratedLabel}
                  </span>
                </div>
              )}
