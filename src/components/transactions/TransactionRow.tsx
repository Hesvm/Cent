...
export function TransactionRow({ transaction, onClick, isLast, isPending, onFrequencyPillClick }: TransactionRowProps) {
  const { name, amount, type, category, frequency, isAutoGenerated } = transaction
  const isIncome = type === 'income'
  const displayName = name.length > MAX_NAME_LENGTH ? name.slice(0, MAX_NAME_LENGTH) + '…' : name

  const showFrequency = frequency !== 'none'
  const recurringLabel = frequency !== 'none' ? frequency.charAt(0).toUpperCase() + frequency.slice(1) : 'Recurring'
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex flex-wrap items-center gap-1">
          ...
          {isAutoGenerated && <CategoryPill label={`↻ ${recurringLabel}`} />}
          {showFrequency && !isAutoGenerated && (
